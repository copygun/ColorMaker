...
class LocalLearningSystem {
  ...
  private analyzeSystematicBias(results: PrintResult[]): DeviationPattern | null {
    ...
    const stdDev = {
      L: Math.sqrt(
        deviations.reduce((sum, d) => sum + Math.pow(d.dL - avgDeviation.L, 2), 0) /
          deviations.length,
      ),
      a: Math.sqrt(
        deviations.reduce((sum, d) => sum + Math.pow(d.da - avgDeviation.a, 2), 0) /
          deviations.length,
      ),
      b: Math.sqrt(
        deviations.reduce((sum, d) => sum + Math.pow(d.db - avgDeviation.b, 2), 0) /
          deviations.length,
      ),
    };
    ...
    const threshold = 0.5;
    if (
      Math.abs(avgDeviation.L) > threshold ||
  ...
  private analyzeColorRangePatterns(results: PrintResult[]): DeviationPattern[] {
    ...
    colorRanges.forEach((range) => {
      const inRange = results.filter((r) => {
        ...
        return (
          (range.minL === undefined || lab.L >= range.minL) &&
      ...
      if (inRange.length >= 3) {
        const avgDev = {
          L:
            inRange.reduce((sum, r) => sum + (r.actualLab.L - r.predictedLab.L), 0) /
            inRange.length,
          a:
            inRange.reduce((sum, r) => sum + (r.actualLab.a - r.predictedLab.a), 0) /
            inRange.length,
          b:
            inRange.reduce((sum, r) => sum + (r.actualLab.b - r.predictedLab.b), 0) /
            inRange.length,
        };
  ...
  private updateCalibrationProfile(patterns: DeviationPattern[]): void {
    ...
    if (validResults.length > 0) {
      profile.averageDeltaE =
        validResults.reduce((sum, r) => sum + r.deltaE, 0) / validResults.length;
    }
   ...
   * 보정된 색상 계산
   */
  applyCorrectionToColor(
    color: LabColor,
    recipeInks?: Array<{ inkId: string; ratio: number }>,
  ): LabColor {
    ...
    // 색상 범위별 보정 적용
    const rangeCorrection = profile.colorRangeCorrections.find(
      (rc) =>
        color.L >= rc.range.minL &&
        color.L <= rc.range.maxL &&
        color.a >= rc.range.minA &&
        color.a <= rc.range.maxA &&
        color.b >= rc.range.minB &&
        color.b <= rc.range.maxB,
    );
  ...
  getStatistics() {
    ...
    // 표준편차
    const variance =
      deltaEs.reduce((sum, de) => sum + Math.pow(de - avgDeltaE, 2), 0) / deltaEs.length;
