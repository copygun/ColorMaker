...
export function useColorCalculation(options: UseColorCalculationOptions = {}) {
  ...
  const [features, setFeatures] = useState<FeatureFlags>(options.features || defaultFeatures);

  const [deltaEWeights, setDeltaEWeights] = useState<DeltaEWeights>(
    options.deltaEWeights || { kL: 1, kC: 1, kH: 1 },
  );

  const [deltaEMethod, setDeltaEMethod] = useState<DeltaEMethod>(options.deltaEMethod || 'E00');
  ...
  const mixingEngine = useMemo(() => {
    ...
    features.USE_XYZ_MIXING,
    features.USE_CATMULL_ROM,
    features.USE_KUBELKA_MUNK,
    calculationMode.mode,
  ]);

  // Delta E 계산 함수 선택
  const calculateDeltaE = useCallback(
    (L1: number, a1: number, b1: number, L2: number, a2: number, b2: number) => {
      switch (deltaEMethod) {
        case 'E00':
  ...
  const calculateOptimizedRecipe = useCallback(
    async (
      ...
      if (Array.isArray(formatted)) {
        return formatted.map((fmt: any) => ({
          ...
          inks: fmt.inks.map((ink: any) => {
            const ratio =
              typeof ink.percentage === 'number' ? ink.percentage : parseFloat(ink.percentage) || 0;
            return {
    ...
    async (
      ...
      if (calibrationProfileId) {
        ...
        console.log('Applied calibration:', {
          original: targetColor,
          corrected: correctedTargetColor,
        });
      }
      ...
      try {
        // 고급 모드 또는 하이브리드 모드
        if (
          calculationMode.mode === 'advanced' ||
          (calculationMode.mode === 'hybrid' && features.USE_PSO_OPTIMIZER)
        ) {
          // Advanced mixing engine에 인쇄 설정 전달
          if (
            'updateSettings' in mixingEngine &&
            typeof mixingEngine.updateSettings === 'function'
          ) {
      ...
      if (features.ENABLE_TAC_CHECK) {
        const tacValidation = inkDB.validateTAC(
          Object.fromEntries(selectedInkIds.map((id, i) => [id, result.ratios[i] * 100])),
          profileId,
        );
  ...
  // A/B 테스트: 두 방법 비교
  const compareCalculations = useCallback(
    async (targetColor: LabColor, selectedInkIds: string[]) => {
      // Legacy 방식
      const legacyEngine = new MixingEngine({ mode: 'lab', interpolation: 'linear' });
      const legacyResult = legacyEngine.optimize(
        targetColor,
      ...
      const modernResult = modernEngine.optimize(
        targetColor,
        selectedInkIds.map((id) => inkDB.getInkLab(id, 100)),
        { method: 'simple' },
      );
