  ...
  if (!correctionHistory || correctionHistory.length === 0) {
    return (
      <div className="pro-card">
        ...
        <div className="pro-card-body" style={{ textAlign: 'center', padding: '40px', color: '#999' }}>
                ...
                <strong>
                  {(correctionHistory.reduce((sum, e) => {
                    const imp = e.originalRecipe?.deltaE 
                      ...
                      : 0;
                    return sum + imp;
                  }, 0) / correctionHistory.length).toFixed(2)}
                </strong>
              </div>
              <div>
                성공률 (ΔE &lt; 1): {' '}
                <strong>
                  {((correctionHistory.filter(e => e.predictedDeltaE < 1).length / correctionHistory.length) * 100).toFixed(0)}%
                </strong>
              </div>
            </div>
          </div>
