...
const ColorCorrectionSection: React.FC<ColorCorrectionSectionProps> = ({
  ...
  useEffect(() => {
    const calculatedDeltaE = ColorScience.calculateDeltaE00(
      ...
      measuredLab.L, measuredLab.a, measuredLab.b
    );
  ...
  const applyCorrectionSuggestion = () => {
    ...
    correctionSuggestion.suggestions.forEach((suggestion: any) => {
      const existingInk = correctedRecipe.inks.find(ink => 
        ink.inkId === suggestion.ink || ink.baseInk === suggestion.ink
      );
  ...
  return (
    <div style={{
      ...
      <div style={{ 
        ...
        <label style={{ fontSize: '14px', color: '#1976d2', fontWeight: 600, marginBottom: '8px', display: 'block' }}>
          목표 색상값:
        </label>
      ...
      <div style={{ marginBottom: '16px' }}>
        <label style={{ fontSize: '14px', color: '#666', fontWeight: 600, marginBottom: '8px', display: 'block' }}>
          인쇄 샘플 측정값:
        </label>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
          <div>
            ...
            <div style={{ fontSize: '13px' }}>
              ...
              {correctionHistory.map((history, index) => (
                <div key={index} style={{
                  ...
                  <div>
                    L: {history.measuredLab.L.toFixed(1)}, 
                    a: {history.measuredLab.a.toFixed(1)}, 
                    b: {history.measuredLab.b.toFixed(1)}
                  </div>
                  <div style={{ 
                    fontWeight: 600,
                    color: history.deltaE <= 1 ? '#4caf50' : history.deltaE <= 2 ? '#ff9800' : '#f44336'
                  }}>
                    {history.deltaE.toFixed(2)}
                  </div>
                  <div style={{ fontSize: '12px' }}>
                    {history.suggestions.length > 0 
                      ? history.suggestions.map(s => `${s.ink} +${s.amount.toFixed(1)}%`).join(', ')
                      : '목표 달성'
                    }
                  </div>
                </div>
              ...
              {correctionHistory.length > 1 && (
                <div style={{
                  ...
                  <strong>개선 추이:</strong> 
                  {' '}ΔE {correctionHistory[0].deltaE.toFixed(2)} → {correctionHistory[correctionHistory.length - 1].deltaE.toFixed(2)}
                  {' '}({((1 - correctionHistory[correctionHistory.length - 1].deltaE / correctionHistory[0].deltaE) * 100).toFixed(0)}% 개선)
                </div>
              )}
            </div>
          )}
