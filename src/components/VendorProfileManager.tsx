                      ...
                      <tr key={profile.id}>
                        ...
                        <td>
                          <div style={{ display: 'flex', gap: '8px' }}>
                            ...
                            <button
                              className="pro-button pro-button-secondary"
                              style={{ padding: '4px 12px', fontSize: '0.875rem', color: '#dc3545', borderColor: '#dc3545' }}
                              onClick={() => {
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '900px', maxHeight: '80vh', overflowY: 'auto' }}>
            ...
            <h3 style={{ marginBottom: '24px' }}>
              ...
              {editingProfile.isComplete ? 
                <span style={{ marginLeft: '12px', fontSize: '0.875rem', color: '#28a745' }}>✅ 완성됨</span> :
                <span style={{ marginLeft: '12px', fontSize: '0.875rem', color: '#dc3545' }}>⚠️ 미완성 (Spot Ink Lab 값 필요)</span>
              }
            </h3>
            ...
            {/* Spot Ink Recipes */}
            {editingProfile.spotInkRecipes && Object.keys(editingProfile.spotInkRecipes).length > 0 && (
              <div>
                <h4 style={{ fontSize: '1rem', marginBottom: '12px' }}>Spot Ink 레시피 및 CIELAB 값</h4>
                <table className="pro-table">
                  <thead>
                    <tr>
                      <th>Spot Ink</th>
                  ...
                  <tbody>
                    {Object.entries(editingProfile.spotInkRecipes).map(([spotInkId, recipeData]) => {
                      const spotInk = inkDB.getInkById(spotInkId);
                      const deltaE = recipeData.measuredLab && recipeData.calculatedLab ?
