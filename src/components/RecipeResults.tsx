          ...
          <tfoot>
            <tr style={{ fontWeight: 600 }}>
              ...
              <td>
                {recipe.inks
                  .filter((ink) => ink.ratio > 0)
                  .reduce((sum, ink) => sum + ink.ratio, 0)
                  .toFixed(1)}
                %
              </td>
              <td colSpan={2}></td>
      ...
      {gamutWarning && gamutWarning.isOutOfGamut && (
        ...
        >
          ...
          {gamutWarning.suggestion && (
            ...
            >
              <div style={{ fontWeight: 'bold', marginBottom: '8px' }}>🎯 제안된 대체 색상</div>
            ...
            >
              <div
                style={{
                  ...
                  height: '100%',
                  background:
                    gamutWarning.confidence > 70
                      ? '#4caf50'
                      : gamutWarning.confidence > 40
                        ? '#ff9800'
                        : '#f44336',
          ...
          <div className="pro-card">
            ...
            >
              <h2 className="pro-card-title">
                ...
                <span
                  className={`status-indicator ${getDeltaEStatusClass(currentRecipe.deltaE)}`}
            ...
            <div className="pro-card-body">
              {currentRecipe.isCorrection && currentRecipe.correctionDate && (
                ...
                >
                  ...
                  <div style={{ fontSize: '14px', color: '#555' }}>
                    <div>
                      보정 적용 시간:{' '}
                      {new Date(currentRecipe.correctionDate).toLocaleString('ko-KR')}
                    </div>
                    {currentRecipe.originalDeltaE && (
                      <div>
                        개선 효과: Delta E {currentRecipe.originalDeltaE?.toFixed(2)} →{' '}
                        {currentRecipe.deltaE.toFixed(2)}
              ...
              <div style={{ marginTop: '12px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {(!currentRecipe.status || currentRecipe.status === RecipeStatus.CALCULATED) &&
                  onSelectRecipe && (
                    <button
                ...
                {currentRecipe.status === RecipeStatus.SELECTED && onUpdateRecipeStatus && (
                  <button
                    onClick={() =>
                      onUpdateRecipeStatus(currentRecipe.id!, RecipeStatus.IN_PROGRESS)
                    }
            ...
            return (
              <div className="pro-card">
                <div className="pro-card-header">
                  <h2 className="pro-card-title">
                    최적화된 레시피{' '}
                    {uniqueRecipes.length === 1 ? '' : `(상위 ${uniqueRecipes.length}개)`}
                  </h2>
                </div>
                  ...
                  {uniqueRecipes.map((recipe, recipeIndex) => (
                    <div
                      ...
                      style={{
                        ...
                        borderBottom:
                          recipeIndex < uniqueRecipes.length - 1 ? '1px solid #e0e0e0' : 'none',
                    ...
                    >
                      ...
                      {/* 작업 시작 버튼 */}
                      {(!recipe.status || recipe.status === RecipeStatus.CALCULATED) &&
                        onSelectRecipe && (
                          <button
